...
class Vehicle extends Go
    ...
    update: () ->
        ...
        resources.camera.position.x = @go.body.body.GetPosition().x * window.physicsScale
        resources.camera.position.y = @go.body.body.GetPosition().y * window.physicsScale

        @updateAudio()

    updateAudio: () ->
        pitch =  @driveWheels[ 0 ].wheel.getForwardVelocity().Length() / 100
        frequencyChange( 440 + ( @driveWheels[ 0 ].wheel.getForwardVelocity().Length() / 100 ) * 1000 )
